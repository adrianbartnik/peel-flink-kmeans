...
public class WeightedRandomSamplerWithoutReplacement<T> extends DistributedRandomSampler<T> {
	...
	public Iterator<IntermediateSampleData<T>> sampleInPartition(Iterator<T> input) {
		...
		while (input.hasNext()) {
			...
			double w = ((IntermediateSampleData)element).getWeight();

			if (index < numSamples) {
				// Fill the queue with first K elements from input.
				queue.add(new IntermediateSampleData<T>(Math.pow(random.nextDouble(), 1 / w), element));
				smallest = queue.peek();
			} else {
				double rand = Math.pow(random.nextDouble(), 1 / w);
